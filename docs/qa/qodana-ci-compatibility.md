...
Scope: Release-readiness QA for Qodana, AGP 9.3.1 compatibility, `continue-on-error` policy, and CI-status visibility. This document records evidence and gaps; it does not change product behavior.
...
- Project AGP: AGP 9.3.1 in `gradle/libs.versions.toml`.
...
- Current AGP 9.3.1 CI Qodana run: NOT RUN.
...
- A GitHub Actions Qodana run completes against this AGP 9.3.1 project without infrastructure/import failures.
...
- The non-blocking AGP 9.3.1 risk is visible in the status name and run summary.
...
1. Trigger the `Qodana` workflow on a branch with current AGP 9.3.1.
2. Confirm the job is named `Qodana Analysis (non-blocking AGP 9.3 risk)`.
3. Confirm `Record Qodana compatibility risk` writes the AGP 9.3.1 warning to the run summary.
...
- Release risk: Qodana remains non-blocking; the prior AGP 9.2.1 CI pass does not verify AGP 9.3.1 compatibility.
...
- Release risk: Qodana AGP 9.3.1 compatibility has no current CI pass, so the workflow remains non-blocking.
